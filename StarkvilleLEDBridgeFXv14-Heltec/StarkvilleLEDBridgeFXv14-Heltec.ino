 ...
 * NeoPixelBus library.
 * 
 * This code has OTA capabilities built in. The ESP32 should be connected to the 
 * T-Mobile Hotspot (T-Mobile//ecobitch). When it is, you can open a browser that is
 * also connected to the same network and navigate to http://esp32.local. This should
 * bring up a login screen (haganwalker//walker4). Once logged in, you can upload a .bin
 * file from the Arduino IDE and upload it without plugging in. 
 * 
...
// These lines are core to the LED functionality.
#include "Arduino.h"
#include "heltec.h"
#include <WS2812FX.h>
...
// These lines add OTA updating capibilities.#include <WiFi.h>
...
"<script>"
    "function check(form)"
    "{"
    "if(form.userid.value=='haganwalker' && form.pwd.value=='walker4')"
    "{"
...
"<form method='POST' action='#' enctype='multipart/form-data' id='upload_form'>"
 ...
 "});"
 "</script>";
// NOW LETS GET BACK TO THE OTHER CODE.
...
#define LED_COUNT 1265                          //DO NOT CHANGE THIS. There are 1264 LEDs on the bridge.
...
// Next comes WIFI setup. 
// We then will initialize the ws2812fx library with default options.
void setup() {
// HERE WE'RE DOING ALL OF THE BOARD SETUP. THIS COMES FIRST.
...
//HERE WE SETUP LORA FOR RECEIVING.
  randomSeed(analogRead(A0));
  Heltec.LoRa.setSyncWord(0xF3);
  Heltec.LoRa.receive();

//HERE WE USE THE SMOOTHED LIBRARY TO AVERAGE 3 ANALOG READINGS FROM THE ULTRASONIC.
// (this reduces false readings)
  mySensor.begin(SMOOTHED_AVERAGE, 3);

// WE USE THIS TO DISPLAY INFO ON THE BUILT IN SCREEN TO LET US KNOW SETUP IS DONE
...
//HERE WE BEGIN THE WIFI SETUP FOR OTA UPDATES.
...
// Wait for connection
  ...
  server.begin();
// THIS IS THE END OF THE WIFI OTA CODE. YOU CAN CHECK THE LAST LINE OF THE MAIN CODE FOR THE server.handleClient() FUNCTION CALL. 
 ...
 */
  ...
  */
    ...
    server.handleClient();                                                          // Thia is the only WIFI OTA Code in the Main Loop. 
}
